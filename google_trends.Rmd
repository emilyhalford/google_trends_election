...
voting_percentages = 
  voting_percentages %>% 
  mutate(
    data = map(state_code, ~(gtrends(keyword = "vote + voting", geo = .x, time = '2016-09-30 2020-09-20')$interest_over_time))
  )
...
voting_percentages =
  ...
  filter(
    date <= "2016-10-16" | date >= "2020-09-01" ## Change to october 4th
    )
